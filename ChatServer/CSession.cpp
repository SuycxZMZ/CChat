...
CSession::CSession(boost::asio::io_context &io_context, CServer *server)
    : _io_context(io_context), _socket(io_context), _server(server),
      _b_close(false), _b_head_pase(false), _user_id(0) {
...
std::string &CSession::GetUuid() { return _session_id; }
...
int CSession::GetUserId() { return _user_id; }

void CSession::SetUserId(int uid) { _user_id = uid; }
  ...
  try {
    if (!error) {
      ...
      std::cout << "----------- send message finish: "
                << std::string(_send_que.front()->_data,
                               _send_que.front()->_total_len)
