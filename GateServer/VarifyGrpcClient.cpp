...
RPCConnectionPool::RPCConnectionPool(std::size_t poolsize, std::string host,
                                     std::string port)
    : _pool_size(poolsize), _host(host), _port(port), _b_stop(false) {
  for (std::size_t i = 0; i < _pool_size; ++i) {
    std::shared_ptr<::grpc::Channel> channel = ::grpc::CreateChannel(
        _host + ":" + port, ::grpc::InsecureChannelCredentials());
...
void RPCConnectionPool::ReturnConnection(
    std::unique_ptr<VarifyService::Stub> context) {
  std::lock_guard<std::mutex> lock(_mtx);
  if (_b_stop) {
    return;
  }
